// https://github.com/englmaxi/zmk-config/blob/main/boards/shields/led_indicator/src/led.c
...
// Max 16 sequences; more in queue will be dropped.
...
static void led_do_blink(struct blink_item blink) {
    for (int i = 0; i < blink.count; i++) {
        ...
        if (blink.sleep_ms > 0) {
            ...
            k_sleep(K_MSEC(CONFIG_INDICATOR_LED_OFF_TIME_MS));
        }
    }
}
...
static void indicate_ble(void) {
    ...
    if (zmk_ble_active_profile_is_connected()) {
        ...
        blink.duration_ms = CONFIG_INDICATOR_LED_ON_TIME_MS * 3;
        blink.count = 1;
    } else if (zmk_ble_active_profile_is_open()) {
        LOG_INF("Profile %d open, slow blinking", profile_index);
        blink.duration_ms = CONFIG_INDICATOR_LED_ON_TIME_MS * 2;
        blink.count = 3;
    } else {
...
static int led_battery_listener_cb(const zmk_event_t *eh) {
    ...
    if (battery_level > 0 && battery_level <= CONFIG_INDICATOR_LED_BATTERY_LEVEL_CRITICAL) {
        ...
        struct blink_item blink = {.duration_ms = CONFIG_INDICATOR_LED_ON_TIME_MS,
                                   .sleep_ms = 0,
                                   .count = 2};
        k_msgq_put(&led_msgq, &blink, K_NO_WAIT);
    }
...
static void indicate_startup_battery(void) {
    ...
    k_sleep(K_MSEC((CONFIG_INDICATOR_LED_ON_TIME_MS + CONFIG_INDICATOR_LED_OFF_TIME_MS) * (blink.count + 1)));
}
#endif
...
extern void led_process_thread(void *d0, void *d1, void *d2) {
    ...
    while (true) {
        ...
        LOG_DBG("Got a blink item from msgq, count %d, duration %d, sleep_ms %d", blink.count, blink.duration_ms, blink.sleep_ms);
