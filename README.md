...
The `build.sh` script wraps the Docker build described in the next section into a single command.
It requires the one-time setup from [Locally using Docker image](#locally-using-docker-image)
(clone ZMK, then run `west init`/`west update`) to be completed first.

    ./build.sh <board> [shield] [extra cmake args...]

The firmware is written to `<shield>.uf2` in the repository root, or `<board>.uf2` if no shield is given.
...
Examples:
    ...
    # nice!nano v2 with the model_x shield
    ./build.sh nice_nano_v2 model_x

    # match the CI build with ZMK Studio support (pass '' to skip the shield argument)
    ./build.sh modelm '' -DSNIPPET=studio-rpc-usb-uart -DCONFIG_ZMK_STUDIO=y

The script aborts if the build fails (no stale `.uf2` is copied) and removes its container
automatically, so it can be re-run at any time.
